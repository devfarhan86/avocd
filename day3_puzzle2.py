    ...
    for w1 in wires1:
        for w2 in wires2:
            if isIntersecting(w1, w2):
                steps = getSteps(wires1[0:wires1.index(w1)])
                steps += getSteps(wires2[0:wires2.index(w2)])
                if isVertical(w1):
                    steps += abs(w1.start.y - w2.start.y)
                    steps += abs(w2.start.x - w1.start.x)
                    intersections.append(Point(w1.start.x, w2.start.y, steps))
                else:
                    steps += abs(w1.start.x - w2.start.x)
                    steps += abs(w2.start.y - w1.start.y)
                    intersections.append(Point(w2.start.x, w1.start.y, steps))

    return intersections

def findMinimumSteps(points):
    minSteps = None
    for point in points:
        steps = point.steps
        if minSteps == None or minSteps > steps:
            minSteps = steps
    return minSteps

positions = []
overlaps = findIntersections(getWires(input1), getWires(input2))
print(findMinimumSteps(overlaps))